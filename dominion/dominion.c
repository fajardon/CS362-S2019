...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
      ...
      return _adventurer(state, currentPlayer, temphand, drawntreasure, z);
    ...
    case smithy:
      ...
      return _smithy(state, handPos, currentPlayer);
    ...
    case minion:
      ...
      return _minion(choice1, choice2, state, handPos, currentPlayer);
    ...
    case tribute:
      ...
      return _tribute(state, currentPlayer, nextPlayer, tributeRevealedCards);
    ...
    case cutpurse:
      ...
      return _cutpurse(state, handPos, currentPlayer);
...
int _adventurer(struct gameState *state, int currentPlayer, int temphand[], int drawntreasure, int z)
{
  int cardDrawn;
...
int _minion(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer)
{
  int i, j;
  //+1 action
  state->numActions++;
...
int _tribute(struct gameState *state, int currentPlayer, int nextPlayer, int tributeRevealedCards[])
{
  int i;
  if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1){
    ...
    if (state->deckCount[nextPlayer] == 0){
      for (i = 0; i < state->discardCount[nextPlayer]; i++){
        state->deck[nextPlayer][i] = state->discard[nextPlayer][i];//Move to deck	    state->deckCount[nextPlayer]++;
