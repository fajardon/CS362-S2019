...
for(j=0; j<25; j++){
  ...
  int currPlayer=whoseTurn(&game);
  //initialize a game state and player cards
  initializeGame(numPlayer, k, seed, &game);
  game.hand[currPlayer][handpos]=smithy;
  ...
  printf("-----------------TESTING CARD: %s in cardEffect %d/25-----------------\n", TESTCARD, j+1);
  //copy the game state to a test case
  memcpy(&test, &game, sizeof(struct gameState));
  //run the function
  i = cardEffect(smithy, 0, 0, 0, &test, handpos, &bonus);
  ...
  printf("-----------------END TESTING CARD: %s in cardEffect %d/25-----------------\n", TESTCARD, j);
}
