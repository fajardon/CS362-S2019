...
int effectAdventure(int currentPlayer, struct gameState *state, int handPos, int *temphand) {
  int drawntreasure = 0;
  int cardDrawn;
  int z = 0;
...
int effectCouncilRoom(int currentPlayer, struct gameState *state, int handPos) {
  //+4 Cards
  for (int i = 0; i < 4; i++) {
	  drawCard(currentPlayer, state);
	}
...
int effectGardens(int currentPlayer, struct gameState *state, int handPos) {
  //+1 Cards
  drawCard(currentPlayer, state);
  return -1;
}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      return effectAdventure(currentPlayer, state, handPos, temphand);
			
    case council_room:
      return effectCouncilRoom(currentPlayer, state, handPos);
    ...
    case gardens:
      return effectGardens(currentPlayer, state, handPos);
    ...
    case smithy:
      return effectSmithy(nextPlayer, state, handPos);
		
    case village:
      return effectVillage(currentPlayer, state, handPos);
